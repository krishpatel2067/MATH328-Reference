...
A function $f(x)$ is \textbf{even} if $f(-x) = f(x)$ and \textbf{odd} if $f(-x) = -f(x)$.

\begin{itemize}
    \item The product of two even functions is \textbf{even}.
    \item The product of two odd functions is \textbf{even}.
    \item The product of an even and an odd function is \textbf{odd}.
\end{itemize}
...
If the functions are \textbf{orthogonal}, then their inner product is 0. Function orthogonality depends on the interval of integration.
